...
def update_yomi(
    ...
    # 方言処理
    if hougen_mode != None:
        kana_list, accent_list = __hougen_patch(
            kana_list, accent_list, pos_list, hougen_mode
        )
...
def __hougen_patch(
    ...
    if "add_youon_a" in hougen_id:
        for i in range(len(sep_kata)):
            ...
            if pos:
                sep_kata[i] = (
                    sep_kata[i][: pos.end()] + "ァ" + sep_kata[i][pos.end() :]
                )  # type:ignore
    ...
    if "add_youon_i" in hougen_id:
        for i in range(len(sep_kata)):
            ...
            if pos:
                ...
                if (
                    len(str(sep_kata[i])) > pos.end() and sep_kata[i][pos.end()] != "ャ"
                ):  # type:ignore
                    sep_kata[i] = (
                        sep_kata[i][: pos.end()] + "ィ" + sep_kata[i][pos.end() :]
                    )  # type:ignore
    ...
    if "add_youon_e" in hougen_id:
        for i in range(len(sep_kata)):
            ...
            if pos:
                sep_kata[i] = (
                    sep_kata[i][: pos.end()] + "ェ" + sep_kata[i][pos.end() :]
                )  # type:ignore
                # ェがアクセント核になる
                if type(pos.end()) == int:
                    sep_acc[i] = str(pos.end())  # type:ignore
    ...
    if "add_youon_o" in hougen_id:
        for i in range(len(sep_kata)):
            ...
            if pos:
                sep_kata[i] = (
                    sep_kata[i][: pos.end()] + "ォ" + sep_kata[i][pos.end() :]
                )  # type:ignore
                # アクセントを頭高型にする。
                if type(pos.end()) == int:
                    sep_acc[i] = "1"  # type:ignore
    ...
    if "1st_mora_tyouon" in hougen_id:
        ...
        if pos:
            ...
            if pos.start() == 1:
                sep_kata[0] = (
                    sep_kata[0][: pos.end()] + "ー" + sep_kata[0][pos.end() :]
                )  # type:ignore
        else:
    ...
    if "1st_mora_sokuon" in hougen_id:
        ...
        if pos:
            ...
            if pos.start() == 1:
                sep_kata[0] = (
                    sep_kata[0][: pos.end()] + "ッ" + sep_kata[0][pos.end() :]
                )  # type:ignore
        else:
    ...
    if "1st_mora_renboin" in hougen_id:
        ...
        if pos:
            sep_kata[0] = (
                sep_kata[0][: pos.end()] + "ゥ" + sep_kata[0][pos.end() :]
            )  # type:ignore
            # アクセントを頭高型に
            if type(pos.end()) == int:
            ...
            if pos:
                sep_kata[0] = (
                    sep_kata[0][: pos.end()] + "ィ" + sep_kata[0][pos.end() :]
                )  # type:ignore
                # アクセントを頭高型に
                if type(pos.end()) == int:
                    sep_acc[0] = "1"  # type:ignore
