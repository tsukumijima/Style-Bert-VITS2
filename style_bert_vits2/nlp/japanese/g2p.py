...
from style_bert_vits2.nlp.japanese.g2p_utils.fugashi import analyze_text_with_fugashi
from style_bert_vits2.nlp.japanese.g2p_utils.yomikata import apply_yomikata_diff
...
def g2p(
    ...
    sep_phonemes = __handle_long([kata_to_phoneme_list(i) for i in sep_kata])
    ...
    # fugashi + UniDic を利用し読み方とアクセントを改善
    if use_fugashi_unidic == True:
        sep_text, sep_kata, sep_phonemes, phone_tone_list = improve_yomi_and_accent(
            sep_text,
            sep_kata,
            sep_phonemes,
            phone_w_punct,
...
def improve_yomi_and_accent(
    ...
    Args:
        sep_text (list[str]): 単語単位の単語のリスト
        sep_kata (list[str]): 単語単位の単語のカタカナ読みのリスト
        sep_phonemes (list[list[str]]): 各単語ごとの音素のリストのリスト
        phone_w_punct (list[str]): sep_phonemes を結合した、punctuation を元のまま保持した音素列
    ...
    for num1 in range(len(kana_list)):
        ...
        cur_acc_hl = convert_accent2hl(cur_kana, cur_accent)
        accent_hl_list += cur_acc_hl

    # new_sep_phonemes: 各単語ごとの音素のリストのリスト
    new_sep_phonemes = __handle_long([kata_to_phoneme_list(i) for i in kana_list])
    ...
    # new_phone_tone_list == 新しい phone_tone_list (そのままの意味)
    new_phone_tone_list = []
...
def kata_to_phoneme_list(text: str) -> list[str]:
    """
